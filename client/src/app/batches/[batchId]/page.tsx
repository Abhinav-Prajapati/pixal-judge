...
function SelectableImage({ imageId, isSelected, onSelect }: { imageId: number; isSelected: boolean; onSelect: () => void; }) {
    return (
        <div className="relative group aspect-square cursor-pointer" onClick={onSelect}>
            <div className={`overflow-hidden rounded-md shadow-md h-full w-full transition-all duration-200 ${isSelected ? 'ring-4 ring-blue-500' : ''}`}>
                <ThumbnailImage src={`${OpenAPI.BASE}/images/thumbnail/${imageId}`} alt={`Image ${imageId}`} />
            </div>
...
function AddImagesModal({ isOpen, onClose, batch, onUpdateComplete }: { isOpen: boolean; onClose: () => void; batch: BatchResponse; onUpdateComplete: () => void; }) {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
            <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-4xl h-[80vh] flex flex-col">
...
// --- MODIFIED: Main Batch Detail Page Component ---
export default function BatchDetailPage({ params }: { params: { batchId: string } }) {
  ...
  const handleRemoveSelected = async () => {
    ...
    if (!window.confirm(`Are you sure you want to remove ${selectedImageIds.size} image(s) from this batch?`)) return;
